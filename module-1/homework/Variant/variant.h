...
namespace task {
    ...
    auto check_implicitly_convertible(int)
    ...
    template <typename From, typename To>
    inline constexpr bool is_implicitly_convertible_v =
            decltype(check_implicitly_convertible<From, To>(0))::value;
    ...
    template <std::size_t Size>
    constexpr std::size_t findTypePos(std::size_t pos, const bool (&founded)[Size]) {
        return (pos == Size) ? -1 : ((founded[pos]) ? pos : findTypePos(pos + 1, founded));
    }
...
// Non-member functions
    ...
    constexpr const variant_alternative_t<I, Variant<Types...>>& Get(Variant<Types...>& v) {
        return Variant<Types...>::template Get<typename TypeAt<I, TypeList<Types...>>::type>(
